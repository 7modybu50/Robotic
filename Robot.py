...
class RRSPState(pomdp_py.State):
    ...
    def __str__(self):
        ...
        self.oRock)) + "\nObserved papers: " + str(self.oPaper) + "\nObserved scissors: " + str(self.oScissor)
    def __hash__(self):
...
class TransitionModel(pomdp_py.TransitionModel):
    ...
    def probability(self, start_state, end_state, action):
        if end_state.myPoints[0] - start_state.myPoints[0] == 1 and action.name != "play_rock":
            ...
            return 0
        elif end_state.myPoints[2] - start_state.myPoints[2] == 1 and action.name != "play_scissor":
            return 0
        elif end_state.oPoints[0] - start_state.oPoints[0] == 1 and action.name != "play_scissor":
            return 0
        elif end_state.oPoints[1] - start_state.oPoints[1] == 1 and action.name != "play_rock":
            return 0
        elif end_state.oPoints[2] - start_state.oPoints[2] == 1 and action.name != "play_paper":
            return 0
        else:
    ...
    def sample(self, state, action):
        probabilities = [self.probability(self.cur_state,state,action) for state in self.get_all_states()]
        remaining = 1 - sum(probabilities)
    ...
    def replace(self, cards, init, end):
        new_cards = cards.copy()
        index = cards.index(init)
        new_cards[index] = end
        return new_cards
    ...
    def get_all_states(self):
        ...
        if("rock" in ss.cards):
            # draw
            states.append(RRSPState(ss.myPoints, ss.oPoints, self.replace(ss.cards, "rock", "rock"), ss.oRock + 2, ss.oPaper, ss.oScissor))
            states.append(
                RRSPState(ss.myPoints, ss.oPoints, self.replace(ss.cards, "rock", "paper"), ss.oRock + 1, ss.oPaper+1,
                          ss.oScissor))
            states.append(
                RRSPState(ss.myPoints, ss.oPoints, self.replace(ss.cards, "rock", "scissor"), ss.oRock + 1, ss.oPaper,
                          ss.oScissor+1))
            # won
            states.append(
                RRSPState([ss.myPoints[0]+1, ss.myPoints[1], ss.myPoints[2]], ss.oPoints, self.replace(ss.cards, "rock", "rock"), ss.oRock + 1, ss.oPaper,
                          ss.oScissor+1))
            states.append(
                RRSPState([ss.myPoints[0] + 1, ss.myPoints[1], ss.myPoints[2]], ss.oPoints,
                          self.replace(ss.cards, "rock", "paper"), ss.oRock, ss.oPaper+1,
                          ss.oScissor + 1))
            states.append(
                RRSPState([ss.myPoints[0] + 1, ss.myPoints[1], ss.myPoints[2]], ss.oPoints,
                          self.replace(ss.cards, "rock", "scissor"), ss.oRock, ss.oPaper,
                          ss.oScissor + 2))
            # lost
            states.append(
                RRSPState(ss.myPoints, [ss.oPoints[0], ss.oPoints[1]+1, ss.oPoints[2]],
                          self.replace(ss.cards, "rock", "rock"), ss.oRock + 1, ss.oPaper+1,
                          ss.oScissor))
            states.append(
                RRSPState(ss.myPoints, [ss.oPoints[0], ss.oPoints[1] + 1, ss.oPoints[2]],
                          self.replace(ss.cards, "rock", "paper"), ss.oRock, ss.oPaper + 2,
                          ss.oScissor))
            states.append(
                RRSPState(ss.myPoints, [ss.oPoints[0], ss.oPoints[1] + 1, ss.oPoints[2]],
                          self.replace(ss.cards, "rock", "scissor"), ss.oRock, ss.oPaper + 1,
                          ss.oScissor+1))
        if("paper" in ss.cards):
            # draw
            states.append(RRSPState(ss.myPoints, ss.oPoints, self.replace(ss.cards, "paper", "rock"), ss.oRock + 1, ss.oPaper+1, ss.oScissor))
            states.append(
                RRSPState(ss.myPoints, ss.oPoints, self.replace(ss.cards, "paper", "paper"), ss.oRock, ss.oPaper+2,
                          ss.oScissor))
            states.append(
                RRSPState(ss.myPoints, ss.oPoints, self.replace(ss.cards, "paper", "scissor"), ss.oRock, ss.oPaper+1,
                          ss.oScissor+1))
            # won
            states.append(
                RRSPState([ss.myPoints[0], ss.myPoints[1]+1, ss.myPoints[2]], ss.oPoints, self.replace(ss.cards, "paper", "rock"), ss.oRock + 2, ss.oPaper,
                          ss.oScissor))
            states.append(
                RRSPState([ss.myPoints[0], ss.myPoints[1]+1, ss.myPoints[2]], ss.oPoints,
                          self.replace(ss.cards, "paper", "paper"), ss.oRock+1, ss.oPaper+1,
                          ss.oScissor))
            states.append(
                RRSPState([ss.myPoints[0], ss.myPoints[1]+1, ss.myPoints[2]], ss.oPoints,
                          self.replace(ss.cards, "paper", "scissor"), ss.oRock+1, ss.oPaper,
                          ss.oScissor+1))
            # lost
            states.append(
                RRSPState(ss.myPoints, [ss.oPoints[0], ss.oPoints[1], ss.oPoints[2]+1],
                          self.replace(ss.cards, "paper", "rock"), ss.oRock + 1, ss.oPaper,
                          ss.oScissor+1))
            states.append(
                RRSPState(ss.myPoints, [ss.oPoints[0], ss.oPoints[1], ss.oPoints[2]+1],
                          self.replace(ss.cards, "paper", "paper"), ss.oRock, ss.oPaper + 1,
                          ss.oScissor+1))
            states.append(
                RRSPState(ss.myPoints, [ss.oPoints[0], ss.oPoints[1], ss.oPoints[2]+1],
                          self.replace(ss.cards, "paper", "scissor"), ss.oRock, ss.oPaper,
                          ss.oScissor+2))
        if("scissor" in ss.cards):
            # draw
            states.append(RRSPState(ss.myPoints, ss.oPoints, self.replace(ss.cards, "scissor", "rock"), ss.oRock + 1, ss.oPaper, ss.oScissor+1))
            states.append(
                RRSPState(ss.myPoints, ss.oPoints, self.replace(ss.cards, "scissor", "paper"), ss.oRock, ss.oPaper+1,
                          ss.oScissor+1))
            states.append(
                RRSPState(ss.myPoints, ss.oPoints, self.replace(ss.cards, "scissor", "scissor"), ss.oRock, ss.oPaper,
                          ss.oScissor+2))
            # won
            states.append(
                RRSPState([ss.myPoints[0], ss.myPoints[1], ss.myPoints[2]+1], ss.oPoints, self.replace(ss.cards, "scissor", "rock"), ss.oRock+1, ss.oPaper+1,
                          ss.oScissor))
            states.append(
                RRSPState([ss.myPoints[0], ss.myPoints[1], ss.myPoints[2]+1], ss.oPoints,
                          self.replace(ss.cards, "scissor", "paper"), ss.oRock, ss.oPaper+2,
                          ss.oScissor))
            states.append(
                RRSPState([ss.myPoints[0], ss.myPoints[1], ss.myPoints[2]+1], ss.oPoints,
                          self.replace(ss.cards, "scissor", "scissor"), ss.oRock, ss.oPaper+1,
                          ss.oScissor + 1))
            # lost
            states.append(
                RRSPState(ss.myPoints, [ss.oPoints[0]+1, ss.oPoints[1], ss.oPoints[2]],
                          self.replace(ss.cards, "scissor", "rock"), ss.oRock + 2, ss.oPaper,
                          ss.oScissor))
            states.append(
                RRSPState(ss.myPoints, [ss.oPoints[0]+1, ss.oPoints[1], ss.oPoints[2]],
                          self.replace(ss.cards, "scissor", "paper"), ss.oRock+1, ss.oPaper + 1,
                          ss.oScissor))
            states.append(
                RRSPState(ss.myPoints, [ss.oPoints[0]+1, ss.oPoints[1], ss.oPoints[2]],
...
class RRSPProblem(pomdp_py.POMDP):
    ...
    def __init__(self,init_true_state, init_belief):
        ...
        agent = pomdp_py.Agent(init_belief,
                               PolicyModel(),
                               TransitionModel(init_true_state),
                               ObservationModel(init_true_state),
                               RewardModel())        
        
        #defining the environment
        env = pomdp_py.Environment(init_true_state, TransitionModel(init_true_state), RewardModel())
...
def initialize_state(end_state = None):
    if end_state is None:
        ...
        return RRSPState([0,0,0], [0,0,0], ["rock", "rock", "paper", "paper", "scissor"], rocks_count, papers_count, scissors_count)
    else:
